...
export async function createReview(token, reservationIdx, reviewData) {
    try {
        const response = await fetch(`${server}/reservation/${reservationIdx}/review`, {
            method: "post",
            headers: {
        ...
        if (!response.ok) {
            ...
            catch (jsonError) { errorResponse = { message: `서버 오류 (HTTP ${response.status})`}}
            throw new Error(errorResponse || "리뷰 작성에 실패했습니다.")
        }
        const data = await response.json()
        return { success: true, data: data, message: "리뷰가 성공적으로 작성되었습니다."}
    } catch (error) {
