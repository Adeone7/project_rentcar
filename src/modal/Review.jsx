...
export default function ReviewModal({ setModal, reservationIdx }) {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      console.log(
        `[Review] ${reservationIdx}번 예약에 대한 리뷰 작성 시도: 별점 ${rating}, 내용 "${comment}`
      );
      ...
      setError("리뷰 작성 중 예상치 못한 오류가 발생했습니다: " + err.message);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="rounded-2xl border-stone-200 bg-white p-6 shadow-sm">
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <textarea
          ...
          className="w-full h-32 p-3 rounded-lg border border-stone-200 text-sm outline-none focus:border-cyan-400
            resize-none"
        />
        ...
        <div className="w-full flex justify-end gap-2 mt-4">
          <button
            ...
            className="rounded-xl border border-stone-200 bg-white px-4 py-2 text-sm
                font-semibold text-stone-700 hover:bg-ston-50"
            disabled={isLoading}
          >
            취소
          </button>
          <button
            ...
            className="rounded-xl bg-cyan-600 px-4 py-2 text-sm font-semi-bold text-white hover:bg-cyan-700"
            disabled={isLoading}
          >
            {isLoading ? "작성중..." : "리뷰 제출"}
          </button>
